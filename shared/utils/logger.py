...
def get_logger(name: str = "quant_system") -> logging.Logger:
    ...
    if _global_logger is None:
        # 创建全局日志器
        quant_logger = QuantLogger(name)
        quant_logger.configure()
        _global_logger = quant_logger.get_logger()
...
def setup_logging(config: Dict[str, Any]) -> logging.Logger:
    ...
    _global_logger = quant_logger.configure(
        level=level,
        log_file=log_file,
        log_dir=log_dir,
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.info(f"操作完成: {operation_name}, 耗时: {duration:.3f}秒")
                return result
            except Exception as e:
                duration = time.time() - start_time
                logger.error(
                    f"操作失败: {operation_name}, 耗时: {duration:.3f}秒, 错误: {e}")
                raise
        return wrapper
    return decorator
