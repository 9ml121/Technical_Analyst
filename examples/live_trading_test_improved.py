...
class ImprovedLiveTradingTest:
    ...
    def train_model(self):
        ...
        # 获取训练数据
        end_date = date.today()
        start_date = end_date - timedelta(days=200)  # 获取200天数据用于训练

        for stock_code in self.test_stocks:
            try:
                print(f"  获取 {stock_code} 训练数据...")
                ...
                print(f"⚠️  获取 {stock_code} 训练数据失败: {e}")
                continue

        if len(stock_data_list) < 3:
            print("❌ 训练数据不足，无法训练模型")
            return False

        # 准备训练数据
        try:
            print(f"  准备训练数据，共 {len(stock_data_list)} 只股票...")
            ...
            # 训练模型
            print("  开始训练模型...")
            self.strategy.train_model(training_data)
            print("✅ 模型训练完成")
            return True
    ...
    def run_test(self, test_days: int = 5):
        """运行测试"""
        print(f"🚀 开始改进的实盘测试 ({test_days} 天)")
        print("=" * 60)
    ...
    def generate_test_report(self):
        ...
        with open("improved_live_trading_test_results.json", "w", encoding="utf-8") as f:
            json.dump(test_results, f, indent=2,
                      ensure_ascii=False, default=str)

        print(f"\n✅ 测试结果已保存到 improved_live_trading_test_results.json")
...
def run_improved_live_trading_test():
    """运行改进的实盘测试"""
    print("🚀 改进的小资金实盘测试系统")
    print("=" * 60)
    ...
    try:
        # 创建测试系统
        test_system = ImprovedLiveTradingTest(test_capital=10000)
...
if __name__ == "__main__":
    success = run_improved_live_trading_test()
