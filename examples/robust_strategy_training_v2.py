...
class RobustStrategyTrainer:
    ...
    def get_odd_months_data(self, years: int = 3) -> Dict[str, List]:
        ...
        print("  主板前20:", len(
            [s for s in top_stocks if str(s).startswith('60') or str(s).startswith('00')]))
        print("  创业板前20:", len(
    ...
    def create_robust_strategy(self):
        ...
        model_config = ModelConfig(
            model_type='gradient_boosting',  # 梯度提升树，适合短期预测
            n_estimators=300,                # 减少树数量，控制过拟合
            max_depth=4,                     # 降低深度，控制过拟合
            learning_rate=0.05,              # 适当提高步长，减少过拟合
            feature_selection='rfe',         # 递归特征消除
            n_features=50,                   # 保留更多特征，捕捉短期爆发信号
            target_horizon=5                 # 预测5日收益率，符合策略需求
        )

        strategy_config = MLStrategyConfig(
            name="短期爆发力策略V2",
            model_config=model_config,
            signal_threshold=0.002,          # 更低信号阈值，确保有信号生成
            confidence_threshold=0.35,       # 更低置信度门槛，确保信号生成
