...
class QuantitativeTradingStrategy:
    ...
    def _initialize_builtin_strategies(self):
        ...
        momentum_strategy = StrategyConfig(
            ...
            buy_rules=[
                ...
                TradingRule("成交量放大", "volume_ratio_5d > 1.5", 0.20, "5日成交量比20日均量放大1.5倍"),
                TradingRule("技术突破", "ma5_ratio > 0.02 and ma_bullish == 1", 0.20, "站上5日线且均线多头排列"),
                TradingRule("相对强度", "relative_strength > 0.05", 0.15, "相对强度指标良好"),
                TradingRule("波动率适中", "volatility_20d < 0.05 and volatility_20d > 0.01", 0.10, "波动率在合理区间"),
                TradingRule("RSI超卖反弹", "rsi > 30 and rsi < 70", 0.10, "RSI在合理区间")
            ],
            sell_rules=[
        ...
        mean_reversion_strategy = StrategyConfig(
            name="均值回归策略",
            buy_rules=[
                TradingRule("超跌反弹", "momentum_5d < -0.08 and momentum_20d > -0.15", 0.30, "短期超跌但中期趋势未破"),
            ...
            sell_rules=[
                ...
                TradingRule("反弹乏力", "momentum_3d < 0.02 and rsi > 50", 0.6, "反弹乏力")
            ],
        ...
        breakout_strategy = StrategyConfig(
            name="技术突破策略",
            buy_rules=[
                TradingRule("价格突破", "price_position > 0.8 and momentum_5d > 0.05", 0.35, "价格突破近期高点"),
                TradingRule("成交量确认", "volume_ratio_5d > 2.0", 0.25, "成交量大幅放大确认突破"),
                TradingRule("均线支撑", "ma_bullish == 1 and ma20_ratio > 0.05", 0.20, "均线多头排列且站稳20日线"),
                TradingRule("MACD金叉", "macd_golden_cross == 1", 0.15, "MACD金叉确认"),
    ...
    def generate_trading_signals(self, stock_data: List[StockData], 
        ...
        if current_positions is None or code not in current_positions:
            buy_signal = self._evaluate_buy_rules(features, code, current_price)
            if buy_signal:
        ...
        if current_positions and code in current_positions:
            ...
            sell_signal = self._evaluate_sell_rules(features, position, code, current_price)
            if sell_signal:
    ...
    def _evaluate_buy_rules(self, features: Dict[str, float], code: str, price: float) -> Optional[TradingSignal]:
        ...
        for rule in self.current_strategy.buy_rules:
            try:
                # 评估规则条件
                condition_result = self._evaluate_condition(rule.condition, features)
        ...
        try:
            ...
            for feature_name, value in features.items():
                if feature_name in condition:
                    eval_condition = eval_condition.replace(feature_name, str(value))
    ...
    def get_strategy_summary(self) -> Dict:
        ...
        return {
            ...
            'buy_rules': [
                {'name': rule.name, 'weight': rule.weight, 'description': rule.description}
                for rule in self.current_strategy.buy_rules
            ],
            'sell_rules': [
                {'name': rule.name, 'weight': rule.weight, 'description': rule.description}
...
if __name__ == "__main__":
    ...
    # 保存策略示例
    strategy_engine.save_strategy('momentum', './config/momentum_strategy.json')
