...
from shared.models.base import TradeRecord, Position, TradingSignal
    ...
    def _execute_sell(self, code: str, price: float, quantity: int, trade_date: date) -> bool:
        ...
        if quantity == position.quantity:
            ...
            position.profit_loss_pct = (
                price - position.avg_cost) / position.avg_cost
    ...
    def process_pending_orders(self, trade_date: date, market_data: Dict[str, float]):
        ...
        for order in self.pending_orders:
            if order['execute_date'] <= trade_date:
                if order['type'] == 'SELL':
                    self._execute_sell(
                        order['code'], order['price'], order['quantity'], trade_date)
...
class QuantitativeBacktestEngine:
    ...
    def run_backtest(self, strategy: QuantitativeTradingStrategy, start_date: date, end_date: date,
    ...
    def _get_market_data(self, trade_date: date, codes: List[str]) -> Dict[str, float]:
        ...
        for code in codes:
            # 生成模拟价格数据
            base_price = 10.0 + hash(code) % 50  # 基于股票代码生成基础价格
            price_change = np.random.normal(0, 0.02)  # 2%的随机波动
            market_data[code] = base_price * (1 + price_change)

        return market_data

    def _check_sell_signals(self, trade_date: date, market_data: Dict[str, float]):
        """检查卖出信号"""
        for code, position in list(self.simulator.positions.items()):
            ...
            if self._should_stop_loss(position, current_price):
                self.simulator.place_sell_order(
                    code, current_price, position.quantity, trade_date)
                continue

            # 生成卖出信号
            stock_data = self._generate_mock_data(code, trade_date)
            if stock_data:
                signals = self.strategy.generate_trading_signals(
                    stock_data, {code: position})

                for signal in signals:
                    if signal.signal_type == "SELL":
                        self.simulator.place_sell_order(
                            code, signal.price, position.quantity, trade_date)
    ...
    def _check_buy_signals(self, trade_date: date, stock_pool: List[str]):
        ...
        for code in stock_pool:
            ...
            if stock_data:
                signals = self.strategy.generate_trading_signals(
                    stock_data, self.simulator.positions)
                ...
                for signal in signals:
                    if signal.signal_type == "BUY":
                        ...
                        if position_size > 0:
                            self.simulator.place_buy_order(
                                code, f"股票{code}", signal.price, position_size, trade_date)

    def _generate_mock_data(self, code: str, end_date: date) -> List[StockData]:
        """生成模拟股票数据"""
        # 生成60天的历史数据
        data = []
        base_price = 10.0 + hash(code) % 50

        for i in range(60):
            date_offset = timedelta(days=60-i)
            data_date = end_date - date_offset
    ...
    def _calculate_backtest_results(self, config: BacktestConfig) -> Dict:
        ...
        return {
            ...
            'trading_days': days,
            'trade_records': [asdict(trade) for trade in self.simulator.trade_records],
            'positions': {code: asdict(pos) for code, pos in self.simulator.positions.items()},
            'portfolio_history': self.simulator.daily_portfolio_value
        }
